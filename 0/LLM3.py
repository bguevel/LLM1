...
HEADERS = {"User-Agent": "MyScraper/1.0 (contact: bguevel@mines.edu)"}
...
def train_from_wiki_titles_file(
    titles_file: str,
    model: Transformer,
    config: Config,
    device: str,
...
def interactive_prompt_loop(
    ...
    loaded = load_weights_if_present(model, device, load_path)
    if loaded and load_path:
        print(f"[ok] Loaded weights from {load_path}")
    elif load_path:
        print(f"[warn] No weights found at {load_path}. You are sampling from random weights.")
    else:
        print("[warn] No load file provided. You are sampling from random weights.")
...
def main():
    ...
    if load_path is not None:
        loaded = load_checkpoint(path=load_path, device=device)
    else:
        loaded = None
    ...
    if loaded is not None:
        ...
        if load_path is not None:
            ...
            print("[warn] No load file provided. Starting fresh.")
        config = Config(
            d_model=256,
            d_hidden=1024,
            d_head=64,
    ...
    while True:
        ...
        if choice in ("q", "quit", "exit"):
            break
        if choice == "1":
            training_menu(model, config, device, save_path)
        elif choice == "2":
            interactive_prompt_loop(model, config, device, load_path)
        else:
